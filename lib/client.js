	...
	factory: (require) => {
		...
		const LABELS = {
			zh: {
				...
				crashTitle: '這張設定卡載入失敗，但其他卡片不受影響。',
				crashDetail: '錯誤訊息：',
			},
			en: {
				...
				crashTitle: 'This settings card failed to load. The other cards are unaffected.',
				crashDetail: 'Error: ',
			},
		};
		...
		// One place that says what went wrong and why it matters, used by both layers below.
		// Failures here must be LOUD: a card that never appears looks exactly like "this
		// plugin has no settings page", which is the failure shape nobody investigates.
		function describeError(error) {
			return error && error.message ? error.message : String(error);
		}
		function report(ctx, message, error) {
			const line = 'chinese-script-policy: ' + message + ': ' + describeError(error);
			try {
				if (ctx && ctx.logger && typeof ctx.logger.warn === 'function') ctx.logger.warn(line);
			} catch { /* a logger that throws must not break anything either */ }
			try { console.error(line, error); } catch { /* no console (tests): ignore */ }
		}

		// Two layers, because they fail differently (both measured on another DSH plugin,
		// 2026-09-20):
		//   * a throw OUT of apply() travels up the plugin tree - the worst case is the whole
		//     tree failing to load (that time: a missing exports["./client"] surfaced as a
		//     dsh-client-modules error that never mentioned the real cause);
		//   * a throw during RENDER is caught by the host for us, but only that slot dies and
		//     it dies BLANK - so this file wraps its own card in a boundary that prints why.
		// The registered function is the boundary's parent and only calls createElement, so it
		// cannot throw before the boundary exists (the first layer the card mentions).
		function apply(ctx) {
			try {
				mount(ctx);
			} catch (error) {
				report(ctx, 'settings card not mounted', error);
			}
		}
		...
		function mount(ctx) {
			...
			// React has no hook equivalent of an error boundary, so this one is a class. It
			// renders the reason IN PLACE of the card: a blank slot is the worst failure
			// shape, because it is indistinguishable from "this plugin has no settings".
			class CardBoundary extends React.Component {
				...
				componentDidCatch(error) {
					report(ctx, 'settings card crashed while rendering', error);
				}
				render() {
					if (this.state && this.state.error) {
						...
						return React.createElement('li', {
							...
							React.createElement('p', {
								key: 'title',
								style: { margin: 0, color: TOKENS.primary, fontSize: '13px', lineHeight: '1.5' },
							}, t('crashTitle')),
							React.createElement('p', {
								key: 'detail',
								style: { margin: '6px 0 0', color: TOKENS.secondary, fontSize: '12px', lineHeight: '1.5', whiteSpace: 'pre-wrap' },
							}, t('crashDetail') + message),
						]);
					}
			...
			ctx.slots.inject('settings.plugin.item', function* () {
				yield ctx.slots.register({
					...
					key: NAMESPACE,
					locale: NAMESPACE,
				}, () => React.createElement(CardBoundary, null, React.createElement(Card)));
			});
		}
